...
@interface EOSetTradeView ()<UITextFieldDelegate>
{
...
//        if ([UIDevice iPhonesModel] == iPhone5)
//        {
//            _paymentAlert.frame = CGRectMake(40, [UIScreen mainScreen].bounds.size.height-KEYBOARD_HEIGHT-KEY_VIEW_DISTANCE-ALERT_HEIGHT, [UIScreen mainScreen].bounds.size.width-80, ALERT_HEIGHT);
//        }
...
//        [_paymentAlert addSubview:_amountLabel];
        ...
        _inputView = [[UIView alloc]initWithFrame:CGRectMake(15, _paymentAlert.frame.size.height-(PAYMENT_WIDTH-30)/6-50, PAYMENT_WIDTH-30, (PAYMENT_WIDTH-30)/6)];
        ...
        _detailLabel = [[UILabel alloc]initWithFrame:CGRectMake(15, maxy+CGRectGetHeight(_inputView.frame)+5, PAYMENT_WIDTH-30, 20)];
        _detailLabel.text = @"两次密码不一致，请重新设置";
        _detailLabel.textAlignment = NSTextAlignmentCenter;
        _detailLabel.textColor = TRZXWalletMainColor;
        ...
        [_sureButton setFrame:CGRectMake((PAYMENT_WIDTH-TITLE_HEIGHT)/2, CGRectGetMaxY(_pwdTextField.frame)+ 20, TITLE_HEIGHT, TITLE_HEIGHT)];
        [_sureButton setTitle:@"确认" forState:UIControlStateNormal];
        [_sureButton setTitleColor:[UIColor darkGrayColor] forState:UIControlStateNormal];
        [_sureButton addTarget:self action:@selector(next) forControlEvents:UIControlEventTouchUpInside];
        _sureButton.titleLabel.font = [UIFont systemFontOfSize:15];
        [_paymentAlert addSubview:_sureButton];
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    if (totalString.length == 6) {
        ...
        if (self.firtPassword.length == 6) {
            if (![self.firtPassword isEqualToString:totalString]) {
