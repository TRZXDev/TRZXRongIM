...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView
    ...
    if (self.users.count && (self.users.count - 1 >= indexPath.row)) {
        ...
        [cell.ivAva setImage:[UIImage RC_BundleImgName:@"RCDGroupSetting_Add"]];
        //[RCDUtilities imageNamed:@"add_members" ofBundle:@"RongCloud.bundle"]
    } else {
        cell.btnImg.hidden = YES;
        cell.gestureRecognizers = nil;
        cell.titleLabel.text = @"";
        [cell.ivAva setImage:[UIImage RC_BundleImgName:@"RCDGroupSetting_Delete"]];
//        [RCDUtilities imageNamed:@"delete_members" ofBundle:@"RongCloud.bundle"]
        //长按显示减号
        UILongPressGestureRecognizer *longPressGestureRecognizer =
        [[UILongPressGestureRecognizer alloc]
         initWithTarget:self
...
- (void)showDeleteTip:(RCDConversationSettingTableViewHeaderItem *)cell {
    if (!self.deleteMenberBool) {
        if (self.isAllowedDeleteMember) {//显示
            self.showDeleteTip = YES;
            [self reloadData];
            self.deleteMenberBool = YES;
        }
    }else{
        if (self.showDeleteTip == YES) {//隐藏
            
            self.showDeleteTip = NO;
            
            [self reloadData];
            
            self.deleteMenberBool = NO;
        }
    }
}
