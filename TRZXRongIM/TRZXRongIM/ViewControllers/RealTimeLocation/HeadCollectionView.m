...
- (instancetype)initWithFrame:(CGRect)frame
    ...
    if (self) {
        ...
        [quitButton setImage:[UIImage RC_BundleImgName:@"RCDSharedLocation_Clock"] forState:UIControlStateNormal];
        ...
        [backButton setImage:[UIImage RC_BundleImgName:@"RCDSharedLocation_Top"] forState:UIControlStateNormal];
    ...
    if (userId && [self getUserIndex:userId] < 0) {
        if ([RCIM sharedRCIM].userInfoDataSource && [[RCIM sharedRCIM].userInfoDataSource respondsToSelector:@selector(getUserInfoWithUserId:completion:)]) {
            [[RCIM sharedRCIM].userInfoDataSource getUserInfoWithUserId:userId completion:^(RCUserInfo *userInfo) {
                if (!userInfo) {
                    userInfo = [[RCDUserInfo alloc] initWithUserId:userId name:[NSString stringWithFormat:@"user<%@>", userId] portrait:nil];
                }
            ...
            RCDUserInfo *userInfo = [[RCDUserInfo alloc] initWithUserId:userId name:userId portrait:nil];
            [self.rcUserInfos addObject:userInfo];
            [self addHeadViewUser: userInfo];
            if (show) {
                [self showUserChangeInfo:[NSString stringWithFormat:@"%@加入...", userInfo.name]];
...
- (BOOL)removeUser:(NSString *)userId showChange:(BOOL)show {
    if (userId) {
        ...
        if (index >= 0) {
            RCDUserInfo *userInfo = self.rcUserInfos[index];
...
- (void)onUserSelected:(UITapGestureRecognizer *)tap {
    ...
    RCDUserInfo *user = self.rcUserInfos[index];
